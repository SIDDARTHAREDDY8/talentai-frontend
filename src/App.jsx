...
const QUESTIONS = {
  ...
  "Frontend Developer":[
    ...
    {id:9,q:"How does the browser render a webpage from HTML to pixels?",ref:"Critical rendering path: parse HTML→DOM, parse CSS→CSSOM, combine→Render Tree, Layout (compute geometry), Paint (pixels), Composite (layers). Reflow triggers layout recalc. Repaint triggers pixel redraw. Use transform/opacity for GPU-composited animations to avoid reflow."},
    {id:10,q:"What is the difference between == and === in JavaScript?",ref:"== uses type coercion (1=='1' is true). === strict equality, no coercion (1==='1' is false). Always use === to avoid subtle bugs. Notable: null==undefined is true, null===undefined is false. NaN is not equal to anything including itself."},
  ],
  "Backend Developer":[
    {id:1,q:"What is the difference between synchronous and asynchronous programming?",ref:"Sync: blocking, sequential execution. Async: non-blocking, uses callbacks/promises/async-await. Async prevents blocking I/O operations. Node.js event loop handles async via libuv. Python uses asyncio. Use async for I/O-bound tasks, threading/multiprocessing for CPU-bound."},
    {id:2,q:"Explain database connection pooling and why it matters.",ref:"Pool maintains reusable DB connections instead of creating new ones per request. Benefits: reduces latency, limits max connections, improves throughput. Config: min/max pool size, timeout, idle timeout. Tools: PgBouncer for PostgreSQL, HikariCP for Java."},
    {id:3,q:"What are the differences between REST and GraphQL?",ref:"REST: resource-based URLs, multiple endpoints, over/under-fetching issues, cacheable. GraphQL: single endpoint, client specifies exact data, no over-fetching, harder to cache. REST simpler for public APIs. GraphQL better for complex, nested data with multiple clients."},
    {id:4,q:"How do you handle database transactions and ensure data integrity?",ref:"ACID: Atomicity (all or nothing), Consistency (valid state), Isolation (concurrent transactions), Durability (persisted). Use BEGIN/COMMIT/ROLLBACK. Isolation levels: Read Uncommitted, Read Committed, Repeatable Read, Serializable. Use optimistic locking for low-contention scenarios."},
    {id:5,q:"What is the difference between horizontal and vertical scaling?",ref:"Vertical: add more power (CPU, RAM) to existing server. Simple but has limits. Horizontal: add more servers, distribute load. Requires stateless design, load balancing, distributed sessions. Horizontal preferred for high availability and cost efficiency at scale."},
    {id:6,q:"Explain message queues and when you would use them.",ref:"Async communication between services via broker (RabbitMQ, Kafka, SQS). Use for: decoupling services, rate limiting, background jobs, guaranteed delivery, event streaming. Patterns: pub/sub, work queues, dead letter queues. Kafka for high-throughput streaming, RabbitMQ for task queues."},
    {id:7,q:"What is rate limiting and how would you implement it?",ref:"Controls request frequency per client. Algorithms: Token Bucket (smooth bursts), Leaky Bucket (constant rate), Fixed Window, Sliding Window. Implement at API gateway or middleware. Store counters in Redis for distributed systems. Return 429 Too Many Requests with Retry-After header."},
    {id:8,q:"How would you design a secure REST API?",ref:"HTTPS everywhere. Auth: JWT/OAuth2, rotate secrets. Input validation and sanitization. Rate limiting. CORS properly configured. SQL injection prevention via parameterized queries. Store passwords with bcrypt. Log security events. Use OWASP Top 10 as checklist."},
    {id:9,q:"What is the N+1 query problem and how do you fix it?",ref:"N+1: 1 query to get list, then N queries for each item's relations. Fix: eager loading (JOIN or include), DataLoader batching (GraphQL), select_related/prefetch_related (Django), with() (Laravel). Profile with query logging to detect. Can reduce 100s of queries to 1-2."},
    {id:10,q:"Explain the difference between SQL JOINs.",ref:"INNER JOIN: matching rows in both tables. LEFT JOIN: all left + matching right (NULLs for no match). RIGHT JOIN: all right + matching left. FULL OUTER JOIN: all rows from both. CROSS JOIN: cartesian product. Use INNER for strict matches, LEFT for optional relationships."},
  ],
  "DevOps Engineer":[
    {id:1,q:"What is the difference between CI and CD?",ref:"CI (Continuous Integration): automatically build and test on every commit. CD (Continuous Delivery): automatically deploy to staging after CI passes. Continuous Deployment: auto-deploy to production. CI catches bugs early. CD reduces deployment risk. Tools: Jenkins, GitHub Actions, GitLab CI."},
    {id:2,q:"Explain Infrastructure as Code and its benefits.",ref:"IaC: manage infrastructure via code (Terraform, CloudFormation, Pulumi). Benefits: version controlled, reproducible, consistent environments, automated provisioning, documentation as code, drift detection. Terraform is cloud-agnostic. Use modules for reusability. State management critical."},
    {id:3,q:"What is the difference between Docker containers and virtual machines?",ref:"VMs: full OS, hypervisor, GBs of disk, minutes to start. Containers: share host OS kernel, isolated process, MBs of disk, seconds to start. Containers more efficient but less isolated. VMs better for strong isolation, different OS. Docker uses namespaces and cgroups for isolation."},
    {id:4,q:"How does Kubernetes handle service discovery and load balancing?",ref:"Service resource provides stable DNS/IP for pod group. kube-proxy manages iptables/ipvs rules. ClusterIP for internal, NodePort for external, LoadBalancer for cloud LB. CoreDNS for DNS-based discovery. Ingress for HTTP routing. Services select pods via label selectors."},
    {id:5,q:"What is blue-green deployment and what are its advantages?",ref:"Two identical environments: Blue (live) and Green (new version). Switch traffic via load balancer after Green passes tests. Advantages: zero downtime, instant rollback (flip back to Blue), full production testing before cutover. Drawback: doubles infrastructure cost during deployment."},
    {id:6,q:"Explain the concept of monitoring and observability.",ref:"Monitoring: tracking known metrics (CPU, error rate). Observability: ability to understand internal state from external outputs. Three pillars: Metrics (Prometheus/Grafana), Logs (ELK Stack), Traces (Jaeger/Zipkin). SLIs/SLOs/SLAs define reliability targets. Alert on symptoms not causes."},
    {id:7,q:"What is a Kubernetes pod and how does it differ from a container?",ref:"Pod is smallest deployable unit in Kubernetes, wraps one or more containers sharing network namespace and storage. Containers in a pod communicate via localhost. Pod has single IP. Use multiple containers in pod for sidecar patterns (logging, proxy). Pods are ephemeral."},
    {id:8,q:"How would you handle secrets management in a DevOps pipeline?",ref:"Never store secrets in code or repos. Use: HashiCorp Vault, AWS Secrets Manager, Kubernetes Secrets (base64, not encrypted by default — use KMS). Inject via env vars or mounted files. Rotate regularly. Audit access. Use IRSA/Workload Identity for cloud credentials. Scan repos with git-secrets."},
  ],
  "Cloud Engineer":[
    {id:1,q:"What is the difference between IaaS, PaaS, and SaaS?",ref:"IaaS: raw infrastructure (EC2, VMs) — you manage OS up. PaaS: managed platform (Elastic Beanstalk, App Engine) — you manage app/data. SaaS: complete software (Gmail, Salesforce) — you just use it. More abstraction = less control but less management overhead."},
    {id:2,q:"Explain AWS VPC and its key components.",ref:"VPC: isolated virtual network. Subnets: public (internet-routable) and private. Internet Gateway: connects VPC to internet. NAT Gateway: private subnets outbound internet. Route Tables: traffic routing rules. Security Groups: stateful instance firewall. NACLs: stateless subnet firewall."},
    {id:3,q:"What is the difference between AWS S3 storage classes?",ref:"Standard: frequent access, low latency. Standard-IA: infrequent access, lower cost. One Zone-IA: single AZ, cheaper. Glacier Instant: archive, millisecond retrieval. Glacier Flexible: minutes-hours retrieval. Deep Archive: lowest cost, 12hr retrieval. Use lifecycle policies to auto-transition."},
    {id:4,q:"How does auto-scaling work in cloud environments?",ref:"Monitors metrics (CPU, memory, custom) via CloudWatch. Scale out (add instances) when threshold exceeded. Scale in (remove instances) when load drops. Cooldown periods prevent thrashing. Target tracking (maintain metric), step scaling (tiered), scheduled scaling (predictable load). ALB distributes to new instances."},
    {id:5,q:"What is the shared responsibility model in cloud security?",ref:"Cloud provider responsible for security OF the cloud (physical, hypervisor, network). Customer responsible for security IN the cloud (OS patches, app security, data encryption, IAM, network config). Varies by service: IaaS customer does more, SaaS provider does more."},
    {id:6,q:"Explain the difference between horizontal pod autoscaling and cluster autoscaling in Kubernetes.",ref:"HPA: scales pod replicas based on CPU/memory/custom metrics. Cluster Autoscaler: adds/removes nodes based on pending pods and node utilization. Use together: HPA scales pods first, CA adds nodes when pods can't be scheduled. VPA adjusts resource requests/limits."},
    {id:7,q:"What is a CDN and how does it improve performance?",ref:"CDN (Content Delivery Network) caches content at edge locations close to users. Reduces latency by serving from nearest PoP. Offloads origin server. Handles DDoS via distributed capacity. CloudFront, Fastly, Cloudflare. Use for static assets, images, video. Cache-Control headers control TTL."},
    {id:8,q:"How would you architect a highly available multi-region application?",ref:"Deploy across 3+ AZs per region. Active-active or active-passive multi-region. Global load balancing (Route53 latency/geolocation). Replicate data (RDS Multi-AZ, DynamoDB Global Tables). Use eventual consistency where possible. Regular DR drills. RTO/RPO define recovery targets."},
  ],
  "Full Stack Developer":[
    {id:1,q:"How do you manage state in a large React application?",ref:"Local state: useState for component-level. Shared state: Context API for simple cases, Redux/Zustand/Jotai for complex. Server state: React Query/SWR for caching/syncing API data. URL state for shareable UI state. Avoid over-engineering — start simple, add complexity only when needed."},
    {id:2,q:"Explain how JWT authentication works end-to-end.",ref:"User logs in → server validates credentials → server signs JWT (header.payload.signature) with secret → returns JWT. Client stores JWT (httpOnly cookie preferred). Client sends JWT in Authorization header. Server verifies signature, extracts claims. Refresh tokens for long sessions. Never store sensitive data in payload."},
    {id:3,q:"What is the difference between server-side rendering and client-side rendering?",ref:"CSR: browser downloads JS, renders in client. Fast after load, poor initial SEO. SSR: server renders HTML, sends to browser. Better SEO, faster first paint. SSG: pre-render at build time, best performance. ISR: incremental static regeneration, hybrid. Next.js supports all four patterns."},
    {id:4,q:"How would you optimize the performance of a full stack web application?",ref:"Frontend: code splitting, lazy loading, image optimization, CDN, caching. Backend: DB indexing, query optimization, caching (Redis), connection pooling. Network: HTTP/2, compression (gzip/brotli), reduce round trips. Measure with Lighthouse, WebPageTest, profilers. Fix biggest impact first."},
    {id:5,q:"Explain how you would design a real-time feature like live notifications.",ref:"Options: WebSockets (bidirectional, persistent), SSE (unidirectional, simpler), Long polling (legacy). WebSockets via Socket.io or native WS API. Backend emits events on trigger. Scale with Redis pub/sub for multiple server instances. Handle reconnection, heartbeats, auth."},
    {id:6,q:"What is CORS and how do you configure it correctly?",ref:"CORS: browser security blocks cross-origin requests by default. Server must respond with Access-Control-Allow-Origin header. Preflight OPTIONS request for non-simple requests. Configure: allowed origins (never * in production with credentials), methods, headers. Credentials require explicit origin, not wildcard."},
    {id:7,q:"How do you handle errors in a full stack application?",ref:"Frontend: error boundaries (React), try/catch, global window.onerror. Backend: centralized error middleware, HTTP status codes, structured error responses. Logging: structured logs (Winston, Pino), error tracking (Sentry). Never expose stack traces to client. Graceful degradation for UX."},
    {id:8,q:"What is the difference between SQL and NoSQL and when do you use each?",ref:"SQL: structured, ACID, relations, schema-on-write. Best for: financial data, complex queries, strong consistency. NoSQL: flexible schema, horizontal scale, eventual consistency. Document (MongoDB) for flexible data, Redis for caching/sessions, Cassandra for time-series. Use both in one app (polyglot persistence)."},
  ],
  "Product Manager":[
    {id:1,q:"How do you prioritize features in a product roadmap?",ref:"Frameworks: RICE (Reach, Impact, Confidence, Effort), ICE scoring, MoSCoW (Must/Should/Could/Won't), Kano model. Align with company OKRs. Balance: customer value, business impact, technical feasibility. Involve engineering early. Use data + qualitative feedback. Communicate rationale clearly."},
    {id:2,q:"Explain the difference between outputs, outcomes, and impact.",ref:"Outputs: what you ship (features, code). Outcomes: behavior/metric changes from outputs (increased retention). Impact: business result from outcomes (revenue growth). Good PMs focus on outcomes, not outputs. OKRs should measure outcomes. Avoid feature factories — validate that features drive outcomes."},
    {id:3,q:"How do you conduct user research for a new feature?",ref:"Methods: user interviews (qualitative, why), surveys (quantitative, how many), usability testing, diary studies, analytics analysis. Jobs-to-be-done framework. 5-8 interviews for saturation. Recruit target users, not convenient users. Synthesize with affinity mapping. Validate assumptions before building."},
    {id:4,q:"What is a product vision and how do you create one?",ref:"Vision: inspiring long-term destination (3-5 years). Must be: customer-centric, differentiated, achievable, motivating. Framework: who is the customer, what problem, why now, why us, what success looks like. Use Amazon PR-FAQ, Geoffrey Moore's positioning template. Strategy bridges vision to roadmap."},
    {id:5,q:"How do you measure product success?",ref:"North Star Metric: single metric capturing core value delivery. Supporting metrics: acquisition, activation, retention, revenue, referral (AARRR). Leading vs lagging indicators. Guardrail metrics to prevent gaming. Set baselines, run experiments (A/B tests), measure lift. Avoid vanity metrics (downloads vs DAU)."},
    {id:6,q:"How do you work with engineers and designers effectively?",ref:"Involve early — engineers in discovery, designers in problem framing. Write clear PRDs with context, not solutions. Use story mapping for shared understanding. Manage scope tradeoffs transparently. Create space for technical feedback on product decisions. Ship together, celebrate together. Retrospectives for continuous improvement."},
    {id:7,q:"What is the difference between a product strategy and a product roadmap?",ref:"Strategy: how you'll achieve the vision — target market, differentiation, key bets, what you won't do. Roadmap: time-sequenced plan of initiatives/themes to execute strategy. Strategy is stable (quarters-years). Roadmap is flexible (monthly). Roadmap without strategy is just a list of features."},
    {id:8,q:"How do you handle conflicting stakeholder priorities?",ref:"Understand motivations and context behind each request. Align on shared company goals and OKRs. Use data to make objective comparisons. Use RICE/ICE scoring to depersonalize prioritization. Communicate trade-offs clearly. Escalate when truly stuck. Build relationships before conflicts arise. Say no gracefully with explanation."},
  ],
  "Cybersecurity Engineer":[
    {id:1,q:"What is the CIA triad in cybersecurity?",ref:"Confidentiality: only authorized access (encryption, access controls). Integrity: data accuracy and completeness (hashing, checksums, digital signatures). Availability: systems accessible when needed (redundancy, DDoS protection, backups). Security decisions involve trade-offs between these three pillars."},
    {id:2,q:"Explain the difference between symmetric and asymmetric encryption.",ref:"Symmetric: same key to encrypt/decrypt (AES-256). Fast, but key distribution problem. Asymmetric: public key encrypts, private key decrypts (RSA, ECC). Solves key distribution, slower. In practice: asymmetric to exchange symmetric key, then symmetric for bulk data (TLS handshake)."},
    {id:3,q:"What is SQL injection and how do you prevent it?",ref:"Attacker injects SQL via input fields to manipulate queries. Example: ' OR '1'='1 in login form. Prevention: parameterized queries/prepared statements (primary), input validation, stored procedures, ORM, least privilege DB accounts, WAF as defense-in-depth. Never build queries via string concatenation."},
    {id:4,q:"What is the OWASP Top 10 and why is it important?",ref:"OWASP Top 10: most critical web app security risks. Current: Broken Access Control, Cryptographic Failures, Injection, Insecure Design, Security Misconfiguration, Vulnerable Components, Auth Failures, Data Integrity Failures, Logging Failures, SSRF. Industry standard checklist for secure development and code review."},
    {id:5,q:"What is a penetration test and how does it differ from a vulnerability scan?",ref:"Vuln scan: automated tool finds known vulnerabilities (Nessus, Qualys). Fast, broad, low context. Pen test: human tester actively attempts to exploit vulnerabilities, chains findings, tests business logic. Phases: recon, scanning, exploitation, post-exploitation, reporting. Manual testing finds what scanners miss."},
    {id:6,q:"Explain zero-trust security architecture.",ref:"Never trust, always verify — no implicit trust based on network location. Every request authenticated and authorized. Principles: verify identity, least privilege access, assume breach, inspect encrypted traffic. Technologies: identity provider, MFA, micro-segmentation, privileged access management, continuous monitoring."},
    {id:7,q:"What is XSS and how do you prevent it?",ref:"Cross-Site Scripting: attacker injects malicious scripts into web pages viewed by others. Types: Reflected, Stored, DOM-based. Prevention: output encoding (HTML entity encoding), Content Security Policy (CSP), HTTPOnly cookie flag, input validation, use frameworks that auto-escape (React, Angular). Never use innerHTML with user input."},
    {id:8,q:"How do you respond to a security incident?",ref:"NIST framework: Preparation, Detection/Analysis, Containment, Eradication, Recovery, Post-Incident. Immediate: isolate affected systems, preserve logs. Identify scope and attack vector. Eradicate threat, patch vulnerability. Restore from clean backups. Write post-mortem, improve detection. Practice with tabletop exercises."},
  ],
  "Mobile Developer":[
    {id:1,q:"What is the difference between React Native and Flutter?",ref:"React Native: JavaScript/JSX, uses native components, Facebook. Flutter: Dart language, custom rendering engine (Skia/Impeller), Google. Flutter: pixel-perfect UI, better performance, larger binary. React Native: JS ecosystem, easier for web devs, OTA updates via CodePush. Both cross-platform iOS+Android."},
    {id:2,q:"Explain the iOS view lifecycle.",ref:"viewDidLoad (one-time setup) → viewWillAppear (before visible) → viewDidAppear (after visible) → viewWillDisappear → viewDidDisappear. For SwiftUI: onAppear/onDisappear. UIApplicationDelegate for app lifecycle: didFinishLaunching, willEnterForeground, didEnterBackground. SceneDelegate in iOS 13+."},
    {id:3,q:"What is the difference between Activity and Fragment in Android?",ref:"Activity: single screen, entry point, manages lifecycle. Fragment: reusable UI component within Activity, own lifecycle, backstack. Use Fragments for: tablet layouts, navigation, reuse. FragmentManager handles transactions. ViewModel survives configuration changes, shared between Fragments. Modern: single-Activity architecture with Navigation component."},
    {id:4,q:"How do you handle offline data and sync in mobile apps?",ref:"Local DB: SQLite (Room for Android, CoreData/GRDB for iOS). Sync strategies: optimistic updates (update UI immediately, sync in background), last-write-wins, conflict resolution. Use WorkManager/BackgroundTasks for sync. Offline-first: app fully functional offline, sync when connected. Handle merge conflicts explicitly."},
    {id:5,q:"What are the key considerations for mobile app performance?",ref:"Render: avoid overdraw, use RecyclerView/LazyColumn for lists, flatten view hierarchy. Memory: avoid leaks (WeakReference, lifecycle-aware), use profiler. Battery: batch network requests, use push over polling, WorkManager for background. App size: ProGuard/R8, on-demand delivery, WebP images. Profile with Instruments/Android Profiler."},
    {id:6,q:"How do you implement push notifications on iOS and Android?",ref:"iOS: APNs (Apple Push Notification service). Register for permissions, get device token, send to server. Android: FCM (Firebase Cloud Messaging). Both: backend sends to respective service, service delivers to device. Use Firebase for unified backend. Handle foreground/background/terminated states differently. Deep linking from notification."},
    {id:7,q:"What is the difference between deep linking and universal links?",ref:"Deep links: custom URL scheme (myapp://path). Easy but hijackable. Universal Links (iOS) / App Links (Android): HTTPS URLs that open app if installed, browser otherwise. Require domain verification file (apple-app-site-association / assetlinks.json). More secure, better UX. Required for many app store features."},
    {id:8,q:"How do you ensure mobile app security?",ref:"Certificate pinning (prevent MITM). Secure storage: Keychain (iOS), Keystore (Android) for credentials. Obfuscate code (ProGuard). No sensitive data in logs. Biometric auth. Root/jailbreak detection. Encrypted local storage. OWASP Mobile Top 10 as checklist. Regular security audits. Secure API communication."},
  ],
};
